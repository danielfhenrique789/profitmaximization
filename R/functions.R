  ...
  for(ticket in tickets){
    tryCatch({
      ...
      if(!is.null(obj[[ticket]])){
        ...
        lmWeek <- lm(week$Adjusted ~ week$Date)
        lmWeek2 <- lm(week2$Adjusted ~ week2$Date)
        lmMonth <- lm(month$Adjusted ~ month$Date)
        lmTri <- lm(trimester$Adjusted ~ trimester$Date)
        corWeek <- c(corWeek,cor(week$Adjusted,week))
        corMonth <- c(corMonth,cor(month$Adjusted,month))
        corTri <- c(corTri,cor(trimester$Adjusted,trimester))
